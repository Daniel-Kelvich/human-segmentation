...
class FPNMobileNet(nn.Module):
    ...
    def __init__(self, output_ch=1, num_filters=128, num_filters_fpn=128, pretrained=True, weights_path=''):
        super().__init__()

        # Feature Pyramid Network (FPN) with four feature maps of resolutions
        # 1/4, 1/8, 1/16, 1/32 and `num_filters` filters for all feature maps.

        self.fpn = FPN(num_filters=num_filters_fpn, pretrained=pretrained, weights_path=weights_path)
...
class FPN(nn.Module):
    ...
    def __init__(self, num_filters=128, pretrained=True, weights_path=''):
        ...
        if pretrained:
            ...
            state_dict = torch.load(weights_path) # add map_location='cpu' if no gpu
            net.load_state_dict(state_dict)
        self.features = deepcopy(net.features)
        ...
        self.enc4 = nn.Sequential(*deepcopy(self.features[11:16]))
